	...
	componentDidUpdate (prevProps, prevState) {
		...
		if (prevState.ModalPlus !== me.state.ModalPlus) {
			...
			//if (me.state.pp == mapping_data.id) {
				console.log(viewpoint.find('.ModalPlus_'+me.state.ModalPlus).length + '====----===');
				$(viewpoint.find('.ModalPlus_'+me.state.ModalPlus)[0]).modal({backdrop:'static'});
				console.log(me.props.level + '==' +me.state.ModalPlus.pp + '==' +me.props.parent.state.ModalPlus.pp+'=='+mapping_data.id);
		//	} else {
	...
	render () {
		...
		switch(me.state.ModalPlus.type) {
			case "alert":
				...
				if (!err_msg) {
					return (			
						<div className={'modal fade ModalPlus ModalPlus_'+ mapping_data.id} tabindex="-1" role="dialog" aria-hidden="true">
			...
			case "popup":
				if (!err_msg) {
					if (me.state.ModalPlus.body) {
						return (			
							<div className={'modal fade ModalPlus ModalPlus_'+ mapping_data.id} tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
							  <div className="modal-dialog" role="document">
